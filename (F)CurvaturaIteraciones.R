...
infLocal <-
  function(modeloGamlss,
    ...
    if (is.null(porcentaje)) {
      stop("Falta ingresar un valor numerico -> [2,5,10]")
    } else if ((is.numeric(porcentaje) &&
                as.integer(porcentaje)) &&
               (porcentaje == 2 || porcentaje == 5 || porcentaje == 10)) {
      nporcentaje <- porcentaje
    } else{
      stop("--Error, valor no admitido--")
    }
    #-------------------------------------------------------------------------#
    #-componentes-#
    #-------------------------------------------------------------------------#
    ...
    #DB
    datosCurvatura <-
      data.frame(porcentaje = 1:modeloGamlss$noObs,
                 Poon = poonpoon)
